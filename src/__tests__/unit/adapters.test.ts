...
import { createGitWorktreeAdapter, GitWorktreeAdapter } from '../../adapters/git-worktree.ts';
...
describe('GitWorktreeAdapter', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'maestro-worktree-test-'));
  ...
  // -- createGitWorktreeAdapter factory function

  describe('createGitWorktreeAdapter', () => {
    test('creates adapter with correct baseDir and maestroDir', () => {
      const adapter = createGitWorktreeAdapter('/some/project');
      // Verify it's an instance and doesn't throw on construction
      expect(adapter).toBeInstanceOf(GitWorktreeAdapter);
    });
  });

  // -- validateBranchSegment is private static, but exercised through create().
  //    create() calls git operations (revparse, get) before validation, so we
  //    need a real git repo for validation to fire instead of a git error.

  describe('branch name validation (via create)', () => {
    let gitDir: string;

    beforeEach(() => {
      const { execSync } = require('child_process');
      gitDir = fs.mkdtempSync(path.join(os.tmpdir(), 'maestro-git-val-'));
      fs.mkdirSync(path.join(gitDir, '.maestro'), { recursive: true });
    ...
    test('rejects feature name with tilde', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat~1', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    });

    test('rejects feature name with caret', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat^2', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    });

    test('rejects feature name with colon', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat:bad', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    });

    test('rejects feature name with question mark', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat?', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    });

    test('rejects feature name with asterisk', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat*', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    ...
    test('rejects feature name with brackets', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat[0]', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    });

    test('rejects feature name with backslash', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat\\bad', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    ...
    test('rejects feature name with double dots', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat..bad', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    ...
    test('rejects feature name with @{', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat@{0}', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    ...
    test('rejects feature name with space', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('feat bad', 'task-1')).rejects.toThrow(
        /Feature name.*contains characters invalid/
      );
    ...
    test('rejects task name with invalid characters', async () => {
      const adapter = createGitWorktreeAdapter(gitDir);
      await expect(adapter.create('good-feat', 'task~1')).rejects.toThrow(
        /Task name.*contains characters invalid/
      );
    });
  });

  // -- get: returns null when worktree doesn't exist

  describe('get', () => {
    test('returns null for non-existent worktree', async () => {
      const adapter = createGitWorktreeAdapter(tmpDir);
      const result = await adapter.get('no-feature', 'no-task');
      expect(result).toBeNull();
    });
  });

  // -- list: returns empty array when no worktrees directory

  describe('list', () => {
    test('returns empty array when .worktrees dir does not exist', async () => {
      const adapter = createGitWorktreeAdapter(tmpDir);
  ...
  // -- commitChanges: returns appropriate result when worktree doesn't exist

  describe('commitChanges', () => {
    test('returns not-committed when worktree path does not exist', async () => {
      const adapter = createGitWorktreeAdapter(tmpDir);
      const result = await adapter.commitChanges('no-feature', 'no-task');
      expect(result.committed).toBe(false);
      expect(result.message).toBe('Worktree not found');
    });
  });

  // -- hasUncommittedChanges: returns false when worktree doesn't exist

  describe('hasUncommittedChanges', () => {
    test('returns false for non-existent worktree', async () => {
      const adapter = createGitWorktreeAdapter(tmpDir);
      const result = await adapter.hasUncommittedChanges('no-feature', 'no-task');
      expect(result).toBe(false);
    });
  });

  // -- getDiff: simple-git throws synchronously when the worktree directory
  //    doesn't exist (before reaching the async try/catch in getDiff).

  describe('getDiff', () => {
    test('throws when worktree directory does not exist', async () => {
      const adapter = createGitWorktreeAdapter(tmpDir);
      await expect(adapter.getDiff('no-feature', 'no-task')).rejects.toThrow(
        /directory that does not exist/
      );
    });
  });

  // -- cleanup: handles missing worktrees dir gracefully

  describe('cleanup', () => {
    test('returns empty removed list when no worktrees exist', async () => {
      const adapter = createGitWorktreeAdapter(tmpDir);
