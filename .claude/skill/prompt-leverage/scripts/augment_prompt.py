...
def upgrade_prompt(raw_prompt: str, task: str | None = None) -> str:
    ...
    return dedent(f"""\
        ...
        <work_style>
        - {"Go broad first to understand the system, then deep where risk is highest." if intensity == "Deep" else "Focus on the specific task. Broaden scope only if needed."}
        - Use first-principles reasoning before making changes.
        - {"Re-check with fresh eyes before finalizing." if intensity != "Light" else "Keep it simple and direct."}
        </work_style>
        ...
        - {"Check for factual grounding and completeness." if intensity != "Light" else "Quick sanity check."}
        - {"Ask: is there a simpler or more elegant approach?" if intensity == "Deep" else ""}
        </verification>
